...
const realtorFees = (listing) => {
  //https://www.geeksforgeeks.org/javascript-s`trip-all-non-numeric-characters-from-string/
  //I used this resource in the Web Programming 1 projects as well
  price = listing.price.replace(/\D/g, "");
  return `$${listing.price <= 450000 ? price * 0.025 : price * 0.02}`;
};
listingIndex = 2; //Feel free to change value to view calculations
console.log(`The realtor fees for the house at ${
  listings[listingIndex].address
} 
            is ${realtorFees(listings[listingIndex])}`);
...
const listingAscendingly = listings.sort(
  (listingA, listingB) =>
    listingA.price.replace(/\D/g, "") - listingB.price.replace(/\D/g, "")
);
for (i = 0; i < listings.length; i++) {
  console.log(listingAscendingly[i].price);
}
...
for (i = 0; i < filteredListings.length; i++) {
  console.log(filteredListings[i].propertySummary.builtIn);
}
