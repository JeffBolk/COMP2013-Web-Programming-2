...
*/
let studentOne = data[0]; //Dot notation
let studentTwo = data[1]; //Square brackets notation
console.log(studentOne.studentName, studentTwo["studentName"]); //Accessing studentName key of the first and second student objects

/*
*3
To add or change data in an object we call the key and assign a value to it
*/
studentOne.tuitionPaid = 13000; //Change tuition paid for the first student
studentTwo.email = "@gmail.com"; //Add email key to the first student object
 ...
 */
//let { id, studentName, dateOfBirth } = studentOne;
...
let { id, studentName, dateOfBirth, ...restOfStudent } = studentOne;
console.log(restOfStudent);
/**
 *6
 Spread operator (...) to spread keys of an object in a new object
 */
//const copyOfStudentOne = studentOne; //This is not a copy, it's a reference to the same object in memory
const copyOfStudentOne = { ...studentOne };
/**
 *7
 ...
 */
console.log(
  `Student Name: ${studentOne.studentName}, Tuition Paid: ${studentOne.tuitionPaid}`
);
/**
 *8
...
function greetStudent(name) {
  console.log(`Hello ${name.studentName}`);
}
...
greetStudentArrow(studentTwo);

//Lambda function declaration method
const greetStudentLambda = (name) => `Hello ${name.studentName}`;
console.log(greetStudentLambda(studentOne));
